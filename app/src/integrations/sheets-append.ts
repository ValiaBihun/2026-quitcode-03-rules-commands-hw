// Додає лід рядком у Google-таблицю. Таблиця — система обліку, тому сюди йдуть
// повні дані ліда (на відміну від сповіщень у месенджери).
...
/** Відповідь Apps Script: {"status":"ok"} або {"status":"<причина>"}. */
...
const sheetsAppend: Integration = {
  ...
  async send(lead: Lead): Promise<Result<void>> {
    ...
    if (parsed.value.status !== "ok") {
      log.error(`sheets-append: lead ${lead.id} not delivered: ${parsed.value.status}`);
